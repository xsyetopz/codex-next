...
fn spawn_agent_common_properties_v2(agent_type_description: &str) -> BTreeMap<String, JsonSchema> {
    BTreeMap::from([
        ...
        (
            ...
            JsonSchema::string(Some(
                "Optional number of turns to fork. Omit or pass blank text for fresh context. Use `none`, `all`, or a positive integer string such as `3` to fork only the most recent turns."
                    .to_string(),
            )),
        ),
        (
...
Omitting `fork_turns`, passing blank text, or passing `fork_turns="none"` starts the subagent with fresh context. Pass `fork_turns="all"` or a positive integer string only when inherited conversation turns are intentional."#
    );
